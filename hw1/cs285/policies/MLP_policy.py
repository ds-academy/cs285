...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        # return > mean, exp(log_std) > normal distributions
        action = distributions.Normal(
    ...
    def update(self, observations, actions):
        ...
        """
        # TODO: update the policy and return the loss -> numpy to tensor
        action_distribution = self.forward(ptu.from_numpy(observations))
        sampled_actions = action_distribution.rsample()  # rsample, sample 차이?
